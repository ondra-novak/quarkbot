...
public:
    ...
    void update_bid(Price price, Quantity amount) {
        if (amount <= 0) _bid_side.erase(price);
        else _bid_side.replace(price, amount);
    }
    void update_ask(Price price, Quantity amount) {
    ...
    void remove_ask_to(Price price) {
        while (_ask_side.begin()->first < price) {
            _ask_side.erase(_ask_side.begin()->first);
        }
    }

    void trim(Price lowest_price, Price highest_price) {
        {
    ...
    void remove_bid_to(Price price) {
...
protected:
    ...
    struct CmpBid {
        bool operator()(Price a, Price b) const {return (a - b).get_rounded() > 0;}
    };
    struct CmpAsk {
        bool operator()(Price a, Price b) const {return (a - b).get_rounded() < 0;}
    };
    ...
    WanderingTree<Price, Quantity, CmpBid> _bid_side = {};    //<bid side
    WanderingTree<Price, Quantity, CmpAsk> _ask_side = {};    //<ask side
};
